...
ACSD-53148 패치는 구성 가능한 동일한 제품을 장바구니에 추가하기 위해 GraphQL에서 두 개의 병렬 요청이 발생하면 동일한 제품 SKU를 사용하는 장바구니에 두 개의 개별 항목이 발생하는 문제를 해결합니다. 이 패치는 다음 경우에 사용할 수 있습니다. [[!DNL Quality Patches Tool (QPT)]](/help/announcements/adobe-commerce-announcements/magento-quality-patches-released-new-tool-to-self-serve-quality-patches.md) 1.1.37이 설치되었습니다. 패치 ID는 ACSD-53148입니다. 이 문제는 Adobe Commerce 2.4.7에서 수정됩니다.
...
1. SKU를 사용하여 구성 가능한 제품 만들기 *테스트* 및 SKU를 사용하는 간단한 제품 *Test-A*.
1. GraphQL을 통해 새 빈 장바구니 만들기(나만의 위치로 위치 변경) [!DNL URL]):
...
1. 동일한 제품을 장바구니에 추가하려면 두 개의 동시 요청을 실행합니다(변경 사항). *cartID* 및 두 요청의 위치):
...
1. 장바구니에서 항목 보기(변경 사항) *cartId* 및 위치):
...
항목이 다음으로 한 번 나열됨 [!UICONTROL qty] = 2.
...
항목이 다음으로 두 번 나열됨 [!UICONTROL qty] = 1.
